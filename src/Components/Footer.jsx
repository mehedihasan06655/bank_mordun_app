  ...
  return(
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
          ...
          <div className="flex-[1.5] w-full flex flex-row justify-between flex-wrap md:mt-0 mt-10">
              {
                footerLinks.map((footerLink)=>(
                  <div key={footerLink.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
                    <h4 className="font-semibold font-poppins text-white text-[18px] leading-[27px]">{footerLink.title}</h4>
      ...
      <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t[#3f3r45]">
        <p className="font-normal text-center font-poppins text-white text-[18px] leading-[27px]">
        ...
        <div className="flex flex-row md:mt-0 mt-6">
          {
            socialMedia.map((social, index)=>(
              ...
              src={social.icon}
              alt={social.id}
              className={`h-[21px] w-[21px] object-contain cursor-pointer ${index !== socialMedia.length - 1 ? "mr-6" : "mr-0"}`}
              />
            ))
          }              
        </div>
